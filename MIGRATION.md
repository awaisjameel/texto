...
-   Introduced `config/twilio.php` for base URLs, credentials, retry settings.
...
Retry/backoff uses existing `Retry::exponential` with values sourced from `config('twilio.retry.*')` (fallback to `texto.retry`).
